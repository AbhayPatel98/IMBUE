...
contract IMBUE_GymProfileSetUp
    ...
    function purchaseMemberShip(address _owner) public payable{
     ...
     struct ClassStruct  {
         ...
         mapping(address=>ClassStruct) private UserOfflineClasses;
         mapping(address=>OnlineStruct) private UserOnlineClasses;
         ClassStruct[] arr;
    ...
    function CreateOnlineClass(string memory _name, uint _time,uint _duration, string memory _description,string memory _thumbnail ) public {
    ...
    function BookClass(address _Owner,uint _ClassId) public returns(uint){
         //require(MemberShipEnd[_Owner][msg.sender]>=block.timestamp,"Purchase subscription");
         string memory mode=_ClassDetails[_ClassId][_Owner].classMode;
         bytes memory Mode=bytes(mode);
         uint Len=Mode.length;
         if(Len==6){
